...
import { AXIS_LABEL_RADIUS, COORDINATE_SCALE } from "./constants.js";
...
function createPossibilityNodes(scene, theme, vectors) {
    const points = [];
    const seen = new Set();
...
function createAxisLines(scene, theme, vectors) {
    const points = [];
...
export function initialiseRonaldSpace(scene, initialTheme, initialVectors) {
    const state = {
        theme: initialTheme,
        labels: [],
    ...
    function addSpaceGeometry() {
        state.nodeMaterial = createPossibilityNodes(scene, state.theme, state.vectors);
        state.nodePoints = scene.children.at(-1);
        state.axisMaterial = createAxisLines(scene, state.theme, state.vectors);
        state.axisLines = scene.children.at(-1);
    }

    function removeSpaceGeometry() {
        ...
        state.labels = [];
        [state.nodePoints, state.axisLines].forEach(object => {
            if (!object) return;
            scene.remove(object);
    ...
    function addAxisLabels() {
        Object.entries(state.vectors).forEach(([letter, vector]) => {
            const axisLabel = createAxisLabel(
                    scene,
                    letter,
                    vector.clone().multiplyScalar(AXIS_LABEL_RADIUS),
                    state.theme.axisLabel
                );
            axisLabel.sprite = scene.children.at(-1);
            state.labels.push(axisLabel);
        });
    }
    ...
    return {
        ...
        setVectors(vectors) {
            ...
            addSpaceGeometry();
            if (state.typefaceReady) addAxisLabels();
        }
    };
}
