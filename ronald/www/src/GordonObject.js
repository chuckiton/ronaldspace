...
const GLOBAL_GORDON_SCALE = 0.8;
const SIZE_FACTORS = [1.75, 1.95, 2.15, 2.35]
    .map(size => size * GLOBAL_GORDON_SCALE);
// The first consonant controls only the diameter of the seed tetrahedron.
// Later additions use this shared reference so a large seed cannot make its
// prism, pointed shard, or counter-tetra grow without bound.
const MANIPULATION_SCALE = SIZE_FACTORS[0];
// R is moderately more longitudinal than the other profiles. Its prism is
// long enough to read as a cut crystal bar without overwhelming the shared
// scale of the other GORDONs.
const AXIAL_ASPECT_RATIOS = [1, 1.85, 2.15, 2.5];
const COUNTER_TETRA_RATIOS = [0, 0.33, 0.67, 1];
const ROTATIONAL_STEPS = [null, (Math.PI * 2) / 3, Math.PI / 2, Math.PI / 3];
const ROTATIONAL_MODES = ["none", "facet", "facet", "crown"];
// N remains a six-pass rotation, but alternating radial amplitudes creates a
// readable threefold crown within the sixfold facet rhythm.
const N_ROTATION_RADIAL_SCALE = 0.72;
const PICK_PROXY_RADIUS = 4.5;
const POINT_MATCH_EPSILON = 0.00001;
const TETRAHEDRAL_CAP_HEIGHT = Math.sqrt(2 / 3);
const PRISM_EXTENSION_RATIO = 1.05;
// A negative adjustment narrows the far prism termination. Keep the resulting
// diameter positive: a literal negative radial scale would only invert the
// triangular section and risks twisting the connecting facets.
const PRISM_DIAMETER_ADJUSTMENT = -0.28;
const PRISM_DIAMETER_SCALE = 1 + PRISM_DIAMETER_ADJUSTMENT;
const N_TIP_CHAMFER_RATIO = 0.22;
...
function stretchAlongAxis(point, axis, aspectRatio) {
    ...
    // longitudinal: the local G axis lengthens as the threefold waist narrows.
...
function tetraSurfaceFaces(rootVertices) {
    return TETRAHEDRON_FACES.map(face => face.map(index => rootVertices[index]));
}

function counterTetraSurface(
    baseFaces,
    counterRatio,
    sourceScale = MANIPULATION_SCALE,
    operationScale = MANIPULATION_SCALE
) {
    const triangles = [];

    baseFaces.forEach(face => {
        const faceCentre = face.reduce(
            (centre, point) => centre.add(point),
            new THREE.Vector3()
        ...
        // than a separate extrusion on every face. At a ratio of one these
        // apexes form a clean tetrahedral cap over each base face.
        const averageEdgeLength = face.reduce((length, point, index) => (
            length + point.distanceTo(face[(index + 1) % face.length])
        ), 0) / face.length;
        const capLength = (averageEdgeLength / sourceScale) * operationScale;
        const apex = faceCentre.addScaledVector(
            faceNormal,
            capLength * TETRAHEDRAL_CAP_HEIGHT * counterRatio
        );

        face.forEach((point, index) => {
            triangles.push(outwardTriangle(
                point,
...
function convexTriangles(points) {
    const geometry = new ConvexGeometry(uniquePoints(points));
    ...
    for (let index = 0; index < position.count; index += 3) {
        triangles.push(outwardTriangle(
            ...
            new THREE.Vector3().fromBufferAttribute(position, index + 1),
...
function axialProfile(rootVertices, axis, operationScale, profileIndex) {
    if (profileIndex === 1) {
        // R inserts a constant triangular prism between the tetrahedral point
        // and its far termination. The G end remains pointed; the opposite
        // end becomes a clean, faceted shoulder.
        const shiftedBase = [1, 3, 2].map(index => (
            scaleRadialDiameter(
                rootVertices[index].clone()
                    .addScaledVector(axis, -operationScale * PRISM_EXTENSION_RATIO),
                axis,
                PRISM_DIAMETER_SCALE
            )
        ));
        const prismPoints = [...rootVertices, ...shiftedBase];
        const baseIndices = [1, 3, 2];
        const faces = [
            [shiftedBase[0], shiftedBase[1], shiftedBase[2]]
        ];

        baseIndices.forEach((index, faceIndex) => {
            const nextFaceIndex = (faceIndex + 1) % baseIndices.length;
            faces.push([
                rootVertices[0],
                rootVertices[index],
                shiftedBase[faceIndex],
                shiftedBase[nextFaceIndex],
                rootVertices[baseIndices[nextFaceIndex]]
            ]);
        });
    ...
    if (profileIndex === 3) {
        // N cuts back the G tip along its three incident edges, producing a
        // shallow triangular shoulder instead of another elongated needle.
        const tip = rootVertices[0];
        const base = [rootVertices[1], rootVertices[3], rootVertices[2]];
        const cap = [];
        [1, 3, 2].forEach(index => {
            cap.push(tip.clone().lerp(rootVertices[index], N_TIP_CHAMFER_RATIO));
        });
        const chamferPoints = [...base, ...cap];
        const faces = [base, cap];
        base.forEach((point, index) => {
            const next = (index + 1) % base.length;
            faces.push([point, base[next], cap[next], cap[index]]);
        });
    ...
    return {
        points: rootVertices,
        rootVertices,
        faces: tetraSurfaceFaces(rootVertices)
    };
}

function pointedBipyramidSurface(
    profile,
    axis,
    sourceScale = MANIPULATION_SCALE,
    operationScale = MANIPULATION_SCALE
) {
    // Third-position G restores the pointed shard: retain the original G tip
    // as one apex, then reflect it through the profile's outward triangular
    // face to create the opposing apex. Choosing from the construction faces
    // (rather than a triangulated hull) keeps R/N profiles watertight and
    // prevents a triangulation artefact from becoming a flat top plate.
    const outwardFace = profile.faces
        ...
        .reduce((best, face) => {
            ...
            if (!best || centre.dot(axis) < best.centre.dot(axis)) {
                return { face, centre };
            }
            return best;
        }, null).face;
    const [first, second, third] = outwardFace;
    const faceCentre = first.clone()
        .add(second)
        .add(third)
        .multiplyScalar(1 / 3);
    const faceNormal = new THREE.Vector3().crossVectors(
    ...
    const sourceApex = profile.rootVertices[0];
    const distanceToFace = Math.abs(
        faceNormal.dot(sourceApex.clone().sub(first))
    );
    const reflectedDepth = distanceToFace * (operationScale / sourceScale);
    const reflectedApex = faceCentre.addScaledVector(
        faceNormal,
        reflectedDepth
    );

    return convexTriangles([
        ...profile.points,
        sourceApex,
        reflectedApex
    ]);
...
function rotationalGeometry(triangles, axis, rotationalStep, rotationalMode) {
    ...
    sourcePoints.forEach(point => {
        ...
        for (let iteration = 0; iteration < iterations; iteration += 1) {
            ...
            const radialScale = rotationalMode === "crown"
                && iteration % 2 === 1
                ? N_ROTATION_RADIAL_SCALE
                : 1;
            ...
            [-halfWidth, halfWidth].forEach(offset => {
                ...
                rotatedPoints.push(
                    axis.clone().multiplyScalar(axialDistance)
                        .addScaledVector(tangent, Math.cos(angle) * radius * radialScale)
                        .addScaledVector(bitangent, Math.sin(angle) * radius * radialScale)
                );
            });
        }
    });
...
function geometryFor(name, vectors, stage = 0) {
    ...
    // The second consonant chooses the axial profile once the first growth
    // step has completed. G is neutral, R adds a prismatic shoulder, D keeps
    // the elongated-point profile, and N ends in a shallow chamfer. R and D
    // both narrow the root tetra as they lengthen it; N remains a
    // neutral tetra with a cut tip so the shoulder stays compact and clear.
    const profileIndex = stage >= 1 ? parameters[1] : 0;
    const aspectRatio = profileIndex === 1 || profileIndex === 2
        ? AXIAL_ASPECT_RATIOS[profileIndex]
    ...
    const rotationalMode = ROTATIONAL_MODES[parameters[3]];
    ...
    const profile = axialProfile(rootVertices, axis, MANIPULATION_SCALE, profileIndex);
    let triangles;

    if (stage >= 2 && parameters[2] === 0) {
        // G in the third position creates a pointed bipyramidal shard by
        // reflecting the original G apex across the outward profile face.
        triangles = pointedBipyramidSurface(
            profile,
            axis,
            bodySize,
            MANIPULATION_SCALE
        ...
        // The third consonant now acts on the already-profiled crystal. This
        // keeps the parameters sequential: profile first, counter-form next.
        triangles = counterTetraSurface(
            profile.faces,
            counterRatio,
            bodySize,
            MANIPULATION_SCALE
    ...
    if (stage >= 3 && rotationalStep !== null) {
        return rotationalGeometry(triangles, axis, rotationalStep, rotationalMode);
    }
